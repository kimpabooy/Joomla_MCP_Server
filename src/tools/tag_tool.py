...
from src.services.joomla_API import tags_service


@mcp.tool()
def get_tags() -> list[dict]:
    """Fetches all tags from Joomla and returns a list of formatted tag data."""
    tags = tags_service.get_joomla_tags(get_token())
    return [{
            "id": tag.get("id"),
...
def get_tag(tag_id: int) -> dict:
    ...
    tag = tags_service.get_joomla_tag(get_token(), tag_id)
    return {
        "id": tag.get("id"),
        "title": tag.get("attributes", {}).get("title"),
        "alias": tag.get("attributes", {}).get("alias")
...
def create_tag(title: str, alias: str) -> dict:
    ...
    try:
        result = tags_service.create_joomla_tag(get_token(), title, alias)
        return {
            "id": result.get("id"),
...
def edit_tag(tag_id: int, title: str, alias: str) -> dict:
    ...
    try:
        result = tags_service.edit_joomla_tag(
            get_token(), tag_id, title, alias)
        return {
            "id": result.get("id"),
            "title": result.get("attributes", {}).get("title"),
            "alias": result.get("attributes", {}).get("alias")
...
def delete_tag(tag_id: int) -> dict:
    ...
    try:
        result = tags_service.delete_joomla_tag(get_token(), tag_id)
        return result
    except Exception as e:
        return {"error": str(e)}
...
def get_tag_items(tag_id: int) -> list[dict]:
    ...
    items = tags_service.get_joomla_tag_items(get_token(), tag_id)
    return [{
            "id": item.get("id"),
...
def get_tag_item(tag_id: int, item_id: int) -> dict:
    ...
    item = tags_service.get_joomla_tag_item(get_token(), tag_id, item_id)
    return {
        "id": item.get("id"),
        "type": item.get("attributes", {}).get("type"),
        "item_id": item.get("attributes", {}).get("item_id")
...
def create_tag_item(tag_id: int, type: str, item_id: int) -> dict:
    ...
    try:
        result = tags_service.create_joomla_tag_item(
            get_token(), tag_id, item_id)
        return {
            "id": result.get("id"),
...
def edit_tag_item(tag_id: int, item_id: int, type: str, new_item_id: int) -> dict:
    ...
    try:
        result = tags_service.edit_joomla_tag_item(
...
def delete_tag_item(tag_id: int, item_id: int) -> dict:
    ...
    try:
        result = tags_service.delete_joomla_tag_item(
            get_token(), tag_id, item_id)
        return result
    except Exception as e:
        return {"error": str(e)}
